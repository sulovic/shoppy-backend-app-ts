...
router.get("/", checkUserRole((minRole = 3000)), async (req, res) => {
  try {
    ...
    const data = await prisma.porudzbine.findMany({
      where: filter,
      orderBy,
      take,
      skip,
...
router.get("/:id", checkUserRole((minRole = 3000)), async (req, res) => {
  try {
    ...
    const data = await prisma.porudzbine.findUnique({
      where: {
        id: id,
      },
    });
...
router.post("/", checkUserRole((minRole = 3000)), async (req, res) => {
  try {
    ...
    const data = await prisma.porudzbine.create({
      data: newData,
    });
...
router.put("/:id", checkUserRole((minRole = 3000)), async (req, res) => {
  try {
    ...
    const data = await prisma.porudzbine.update({
      where: {
        id: id,
      },
      data: newData,
...
router.delete("/:id", checkUserRole((minRole = 5000)), async (req, res) => {
  try {
    ...
    const existingData = await prisma.porudzbine.findUnique({
      where: {
        id: id,
      },
    });
    ...
    const data = await prisma.porudzbine.delete({
      where: {
        id: id,
      },
    });
